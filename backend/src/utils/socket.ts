...
export const initializeSocket = (httpServer: HttpServer) => {
  ...
  // --- we will store the user ID in the socket
  ...
  // --- it is the event that is triggered when a new client connects to the server ---
    ...
    socket.on(
      ...
      async (data: { chatId: string; text: string }) => {
        try {
          ...
          // --- (испускать) emit to chat room (for users inside the chat)
          io.to(`chat:${chatId}`).emit("new-message", message);

          // --- also emit (испускать) to participants personal rooms (for chat list view)
